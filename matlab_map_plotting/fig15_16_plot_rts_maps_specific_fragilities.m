...
%   - no cut-off is considered. i.e all sites with PGA_475 > 0 are included
...
lit_file = "rts_literature.csv";
save_name = "data_out\DE_rt_pga_lin_median";
figure_name = "rt_pga_map_median";
fig_folder = "C:\Users\Nicholas Clemett\Documents\risiko-basierte-erdbebenkarte\Bearbeitung\new_figures";

% list of indentifiers of fragility curves to use
ids = ["douglas", "luco-2"];
% ids = ["silva" "douglas" "vac-2" "kha-2"];
...
for ii = 1:1:length(ids)
    ...
    for coord = 1:maxcoord
        ...
        [pga_risk(coord), scs, ~, beta_tot(coord)] = rtim_linear(s.k1, s.k0, s.pga_475_median, ...
                                                  lambda_t, m_theta, c_theta, m_beta, c_beta, ...
                                                  beta_haz, beta_cap, tol, max_iter, print); 
